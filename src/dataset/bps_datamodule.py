...
class BPSDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        train_csv = os.path.join(self.train_dir, self.train_csv)
        save_tiffs_local_from_s3(self.s3_client, self.bucket_name, self.s3_path, train_csv, self.train_dir)


        # call the save_tiffs_local_from_s3 function to download tiffs from the
        # val_csv file.      
         
        val_csv = os.path.join(self.val_dir, self.val_csv)
        save_tiffs_local_from_s3(self.s3_client, self.bucket_name, self.s3_path, val_csv, self.val_dir)  
        ...
        if self.test_csv is not None:
            ...
            test_csv = os.path.join(self.test_dir, self.test_csv)
            save_tiffs_local_from_s3(self.s3_client, self.bucket_name, self.s3_path, test_csv, self.test_dir)
